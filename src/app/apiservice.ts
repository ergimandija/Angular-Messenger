...
export class ApiService {
  ...
  constructor(private httpClient: HttpClient) { 
    ...
    this._loginStatus = (statusString!=null)? signal(JSON.parse(statusString)) :signal(initialLoginStatus);
    console.log(this._loginStatus());
    this.loginStatus = this._loginStatus.asReadonly();
   }
    ...
    userData.subscribe(data => {
      if (data.error) {
        ...
        if (data.id != null && data.token != null) {
          ...
          localStorage.setItem("loginStatus", JSON.stringify(this._loginStatus()));
